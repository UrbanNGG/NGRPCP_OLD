...
$curanswer = escape_string($_POST['curanswer']);
...
$gtalk = escape_string($_POST['gtalk']);
$paypal = escape_string($_POST['paypal']);
...
{
	if($_POST['optout'] == 1) mysql_query("UPDATE `accounts` SET `EmailOptOut` = 1 WHERE `id` = '$userid'");
	else mysql_query("UPDATE `accounts` SET `EmailOptOut` = 0 WHERE `id` = '$userid'");
...
if($action == "secquestion") {
	if(isset($_POST['curanswer']))
	{
		...
		if($enc_curanswer == $seccheck['answer'])
		{
			...
			<br><br>
			If you made this request and would like to approve this request for use on the account, please visit this link: <a href="http://127.0.0.1/cp//confirm.php?token='.$token.'&type=2">http://127.0.0.1/cp//confirm.php?token='.$token.'&type=2</a>
			<br /><br />
		...
		$message = 'An account on Next Generation Gaming is requesting to authorize the use of a security question and answer. The question requested is "'.$question.'".
		<br /><br />
		If you made this request and would like to approve this request for use on the account, please visit this link: <a href="http://127.0.0.1/cp//confirm.php?token='.$token.'&type=1">http://127.0.0.1/cp//confirm.php?token='.$token.'&type=1</a>
		<br /><br />
		If this request was not made by you, you can ignore this email and the request will expire in 24 hours from the time of the request being submitted.';
		SendMail($inf['Email'], $inf['Username'], "Your security question requires approval!", $message);
		echo '<meta http-equiv="refresh" content="0;url=../index.php?p=settings&n=2">';
...
if($action == "update_gtalk") {
	if (preg_match("%[A-Za-z0-9._\%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}%", $gtalk)) {
		mysql_query("UPDATE `cp_stat` SET `gtalk` = '$gtalk' WHERE `user_id` = '$userid';");
...
{
	mysql_query("INSERT INTO `sig_stats` (`user_id`, `enabled`) VALUES ('$inf[id]', '1')");
...
if($action == "sig_update")
{
	...
	mysql_query("UPDATE `sig_stats` SET `$querypart` = 0 WHERE `user_id` = $inf[id]");
	mysql_query("UPDATE `sig_stats` SET `$fields` = 1 WHERE `user_id` = $inf[id]");
	echo '<meta http-equiv="refresh" content="0;url=../index.php?p=sig">';
}

if($action == "sig_enable")
{
	if($enable == 1) mysql_query("UPDATE `sig_stats` SET `enabled` = '0' WHERE `user_id` = '$inf[id]'");
	else if($enable == 0) mysql_query("UPDATE `sig_stats` SET `enabled` = '1' WHERE `user_id` = '$inf[id]'");
	echo '<meta http-equiv="refresh" content="0;url=../index.php?p=sig">';
}
?>