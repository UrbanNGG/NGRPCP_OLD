...
{
	echo '<meta http-equiv="refresh" content="0;url=http://127.0.0.1/cp//login.php">';
	session_destroy();
	exit();
}
...
function escape_string($str) {
	$str = mysql_real_escape_string(stripslashes(trim($str)));
	return $str;
}
...
function CheckName($username) {
	...
	$name = mysql_num_rows($query);
	return $name;
}

function GetName($id) {
	$query = mysql_query("SELECT `Username` FROM `accounts` WHERE `id` = '$id'");
...
function GetLastIP($id) {
	$query = mysql_query("SELECT `IP` FROM `accounts` WHERE `id` = $id");
...
function GetAdminLevel($userid) {
	$query = mysql_query("SELECT `AdminLevel` FROM `accounts` WHERE `id` = '$userid'");
	$result = mysql_fetch_array($query);
	return $result[0];
}

function GetHelperLevel($userid) {
	$query = mysql_query("SELECT `Helper` FROM `accounts` WHERE `id` = '$userid'");
	$result = mysql_fetch_array($query);
	return $result[0];
}

function IsDisabled($userid) {
	$query = mysql_query("SELECT `Disabled` FROM `accounts` WHERE `id` = '$userid'");
	$result = mysql_fetch_array($query);
	return $result[0];
}

function Biz($id, $field) {
	$id = $id + 1;
	$query = mysql_query("SELECT $field FROM `businesses` WHERE `id` = $id");
	$result = mysql_fetch_array($query);
	return $result[0];
}

function GetMassEmailRecepients($id)
{
...
function CalculateTotalWealth($id) {
	$username = GetName($id);
	$wealthquery = mysql_query("SELECT `Money`, `Bank`, `Apartment`, `Apartment2` FROM `accounts` WHERE `id` = $id");
	$wealth = mysql_fetch_array($wealthquery);
	if($wealth['Apartment'] > 0) {
		$hwealthquery = mysql_query("SELECT `SafeMoney` FROM `houses` WHERE `id` = $wealth[Apartment] + 1");
		$hwealth = mysql_fetch_array($hwealthquery);
	}
	if($wealth['Apartment2'] > 0) {
		$hwealth2query = mysql_query("SELECT `SafeMoney` FROM `houses` WHERE `id` = $wealth[Apartment2] + 1");
		$hwealth2 = mysql_fetch_array($hwealth2query);
	}
	$totalwealth = $wealth['Money'] + $wealth['Bank'] + $hwealth['SafeMoney'] + $hwealth2['SafeMoney'];
	return number_format($totalwealth);
}

function MapLocation($player,$id,$x,$y,$owner) {
	$playerquery = mysql_query("SELECT `Apartment`, `Apartment2` FROM `accounts` WHERE `Username` = '$player'");
	$playerarray = mysql_fetch_array($playerquery);
	$housequery = mysql_query("SELECT * FROM `houses` WHERE `id` = $playerarray[Apartment] + 1 OR `id` = $playerarray[Apartment2] +1");
...
function LastOnline($userid) {
	$query = mysql_query("SELECT `LastLogin` FROM `accounts` WHERE `id` = '$userid'");
...
function GetFacName($facid) {
	$facquery = mysql_query("SELECT `Name` FROM `groups` WHERE `id` = $facid + 1");
	$fac = mysql_fetch_array($facquery);
	return $fac[0];
}

function GetFamName($famid) {
	$famquery = mysql_query("SELECT `Name` FROM `families` WHERE `id` = $famid");
...
function getNotices($user) {
	$pid = $inf['id'];    
	// Ban Appeals
	$banaplquery = mysql_query("SELECT * FROM `ban` WHERE `status`='4' AND `admin`='$user' ORDER BY `date_added` ASC;");
	$bancount = mysql_num_rows($banaplquery);
	// Flags
	$flagquery = mysql_query("SELECT * FROM `flags` WHERE `id`='$pid' ORDER BY `time` ASC;");
	$flagcount = mysql_num_rows($flagquery);
}
*/

function mInfo($userid) {
	$checkAd = mysql_query("SELECT `AdminLevel`, `Helper` FROM `accounts` WHERE `id`='".$userid."'");
	$cAd = mysql_fetch_array($checkAd);
	if ($cAd['AdminLevel'] >= 2 || $cAd['Helper'] >= 2) {
		if($cAd['AdminLevel'] >=2) {
		$minfoq = mysql_query("SELECT `timezone`, `gtalk` FROM `cp_stat` WHERE `user_id`='".$userid."' AND `type` = '1'");
		} else {
		$minfoq = mysql_query("SELECT `timezone`, `gtalk` FROM `cp_stat` WHERE `user_id`='".$userid."' AND `type` = '3'");
		}
		$minfo = mysql_fetch_array($minfoq);
		if ($minfo['gtalk'] == 'N/A') { echo '<meta http-equiv="refresh" content="0;url=http://'.$_SERVER['SERVER_NAME'].'/index.php?p=settings">'; }
		if ($minfo['timezone'] == '') { echo '<meta http-equiv="refresh" content="0;url=http://'.$_SERVER['SERVER_NAME'].'/index.php?p=settings">'; }  
	}
}
...
function GetStoreStatus($inf) {
	$store = mysql_query("SELECT `store_online` FROM `cp_store_manage` WHERE `id` = '1'");
	$result = mysql_fetch_array($store);
	$status = 'ON';
	return $status;
}
...
function AddToCart($user, $userip, $packid) {
	...
	$duplicatePacks = mysql_query("SELECT `cart_pack_id` FROM `cp_store_cart` WHERE `customer_id`='$user'");
	$dupPack = mysql_num_rows($duplicatePacks);
	if ($dupPack == 1) {
	...
	$findPrevCart = mysql_query("SELECT `cart_id` FROM `cp_store_cart` WHERE `customer_id`='$user' AND `cart_id`='1' LIMIT 1");
	$prevCart = mysql_num_rows($findPrevCart);
	...
	// If there is one, run..
	if ($prevCart == 1) {
	$prevCartID = mysql_query("SELECT `cart_id`,`date_item_added` FROM `cp_store_cart` WHERE `customer_id`='".$user."' ORDER BY `date_item_added` DESC LIMIT 1;");
	$cartId = mysql_fetch_array($prevCartID);
	$cart = $cartId['cart_id'];
	$addProduct = mysql_query("
...
function RemoveFromCart($user, $cartId) {
	$removeq = mysql_query("DELETE FROM `cp_store_cart` WHERE `customer_id`='".$user."' AND `cart_id`='".$cartId."';");
}

function GetStorePrice($packid) {
	$qprices = mysql_query("SELECT `total_price` FROM `cp_store_manage` WHERE `pack_id`='".$packid."';");
	$prices = mysql_fetch_array($qprices);
	$price = $prices['total_price'];
	return $price;
}

function GetOldPrice($packid) {
	$qprices = mysql_query("SELECT `old_price` FROM `cp_store_manage` WHERE `pack_id`='".$packid."';");
	$prices = mysql_fetch_array($qprices);
...
function GetPackPicture($packid) {
	$qpictures = mysql_query("SELECT `pack_picture` FROM `cp_store_manage` WHERE `pack_id`='".$packid."';");
	$pictures = mysql_fetch_array($qpictures);
	$picture = $pictures['pack_picture'];
	return $picture;
}

function GetTotalTokens($packid) {
	$qtokens = mysql_query("SELECT `total_tokens` FROM `cp_store_manage` WHERE `pack_id`='".$packid."';");
	$tokens = mysql_fetch_array($qtokens);
	$token = $tokens['total_tokens'];
	return $token;
}

function GetAdditionalTokens($packid) {
	$qtokens = mysql_query("SELECT `additional_tokens` FROM `cp_store_manage` WHERE `pack_id`='".$packid."';");
	$tokens = mysql_fetch_array($qtokens);
...
function GetCartSize($userid) {
	$userCart = mysql_query("SELECT * FROM `cp_store_cart` WHERE `customer_id`='".$userid."';");
	$cart = mysql_num_rows($userCart);
	return $cart;
}

function GetCartPrice($user) {
	$userCart = mysql_query("SELECT SUM(total_price) FROM `cp_store_cart` INNER JOIN `cp_store_manage` WHERE `customer_id`='".$user."' AND `cart_pack_id` = `pack_id`;");
	$cartItems = mysql_fetch_array($userCart);
	$total = $cartItems['SUM(total_price)'];
	return $total;
}

function GetCartTokens($user) {
	$userCart = mysql_query("SELECT SUM(total_tokens + additional_tokens) as total_tokens FROM `cp_store_cart` INNER JOIN `cp_store_manage` WHERE `customer_id`='".$user."' AND `cart_pack_id` = `pack_id`;");
	$cartItems = mysql_fetch_array($userCart);
...
function ProcessOrder($userid) {
	...
	if(isset($userid)) {
		$sOrder = mysql_query("INSERT INTO `cp_store_orders` (`customer_id`) VALUES (`$userid`);");
		$so = mysql_num_rows($sOrder);
	}
	// Process order
	if ($so == 1) {
...
function GetShiftName($id) {
	$query = mysql_query("SELECT `shift` FROM `cp_shift_blocks` WHERE `shift_id` = $id");
	$array = mysql_fetch_array($query);
	return $array['shift'];
}

function GetShiftIDByHour($hour) {
	$query = mysql_query("SELECT `shift_id` FROM `cp_shift_blocks` WHERE `time_start` = '$hour'");
	$array = mysql_fetch_array($query);
