...
{		
	...
	if ($date == date('j')) {
		echo '<td style="background:#fffdbc"><a href="user.php?p=assignview&y='.date ('l', $timestamp + ($i * (60 * 60 * 24))).'&d='.$assigndate.'">'.date('j', $timestamp + ($i * (60 * 60 * 24))).'</a></td>';
	} else {
		echo '<td><a href="user.php?p=assignview&y='.date ('l', $timestamp + ($i * (60 * 60 * 24))).'&d='.$assigndate.'">'.date('j', $timestamp + ($i * (60 * 60 * 24))).'</a></td>';
	}
    $j++;
    if($j == 7)
    {
